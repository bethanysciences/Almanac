...
RTC_DS3231 rtc;
#define USE_USLDT           true
char LOC[3][4]              = {"LST", "LDT"};
#define GMT_OFFSET          -5
...
bool isLDT(const DateTime& curLST) {
   if(!USE_USLDT) return false;             // if US daylight savings used contiunue
   int y = curLST.year() - 2000;
   int x = (y + y/4 + 2) % 7;

   if(curLST.month() == 3 &&                // Test LDT BEGIN on 2nd Sunday of March @ 2:00 AM
      curLST.day() == (14 - x) && 
      curLST.hour() >= 2)                   // Later than 2am
      return true;
   ...
   if(curLST.month() == 11 &&               // Test LDT: ENDS on 1st Sunday of Nov @ 2:00 AM
      curLST.day() == (7 - x) && 
      curLST.hour() >= 2) 
      return false;
